...
/// Safe public read entry point.
///
/// Model-supplied requested capabilities are intent, never effective authority.
/// Without an independently minted kernel authority this entry point fails closed.
pub fn read_file(
    action: &AgentAction,
    workspace: &Workspace,
...
/// Trusted read entry point used by ForgeCore execution paths.
...
mod tests {
    ...
    #[test]
    fn public_read_fails_closed_without_kernel_authority() {
        let dir = tempfile::tempdir().unwrap();
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        std::fs::write(dir.path().join("a.txt"), b"hello").unwrap();
        let err = read_file(&action_with_path("a.txt"), &ws).unwrap_err();
        assert!(matches!(err, ExecutionError::CapabilityDenied));
    }

    #[test]
    fn successful_read_returns_content_and_hash_with_kernel_authority() {
        let dir = tempfile::tempdir().unwrap();
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        std::fs::write(dir.path().join("a.txt"), b"hello").unwrap();
        let result = read_with_authority(&action_with_path("a.txt"), &ws).unwrap();
    ...
    fn missing_file_is_rejected() {
        let dir = tempfile::tempdir().unwrap();
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        let err = read_with_authority(&action_with_path("missing.txt"), &ws).unwrap_err();
    ...
    fn directory_is_rejected() {
        ...
        let err = read_with_authority(&action_with_path("sub"), &ws).unwrap_err();
    ...
    fn oversized_file_is_rejected() {
        ...
        let err = read_with_authority(&action_with_path("big.txt"), &ws).unwrap_err();
    ...
    fn traversal_is_rejected() {
        let dir = tempfile::tempdir().unwrap();
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        let err = read_with_authority(&action_with_path("../outside.txt"), &ws).unwrap_err();
    ...
    fn unauthorized_path_is_rejected() {
        ...
        let err = read_with_authority(&action_with_path(&outside.display().to_string()), &ws)
            .unwrap_err();
    ...
    fn symlink_escape_is_rejected() {
        #[cfg(unix)]
        {
            ...
            let ws = Workspace::new(&root, 4096).unwrap();
            let err = read_with_authority(&action_with_path("link.txt"), &ws).unwrap_err();
    ...
    fn denied_authority_is_rejected_even_when_requested() {
        ...
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        std::fs::write(dir.path().join("a.txt"), b"hello").unwrap();
        let action = action_with_path("a.txt");
        let err = read_file(&action, &ws).unwrap_err();
        assert!(matches!(err, ExecutionError::CapabilityDenied));
    }

    #[test]
    fn missing_path_field_is_rejected_with_kernel_authority() {
        let dir = tempfile::tempdir().unwrap();
        let ws = Workspace::new(dir.path(), 4096).unwrap();
        let mut action = base_action();
        action.payload = json!({});
        let err = read_with_authority(&action, &ws).unwrap_err();
    ...
    fn high_risk_read_requires_trusted_grant() {
        ...
        let authority = ExecutionAuthority::from_trusted_capabilities([Capability::ReadFile]);
        assert!(matches!(
            read_file_authorized(&action, &ws, &authority, &AuthorizationGrant::none())
                .unwrap_err(),
